...
# --- STYLIZACJA CSS (z dodatkami dla logowania) ---
def local_css():
    st.markdown("""
    <style>
        /* Wymuszenie ciemnego tła dla całej aplikacji */
        .stApp {
        ...
        div[data-testid="stDataFrameDataLayerContainer"] { background-color: #262730 !important; }
        ...
        @media only screen and (max-width: 600px) {
            ...
            button[data-baseweb="tab"] { font-size: 14px !important; padding: 4px 8px !important; }
...
def check_password():
    ...
    st.title("💍 Witaj w Menadżerze Ślubnym Premium")
    st.write("Aby kontynuować, podaj swój klucz produktu otrzymany przy zakupie.")
    password_input = st.text_input("Klucz Produktu:", type="password")
    if password_input:
        if password_input == "Wesele":
            st.session_state["password_correct"] = True
            st.rerun()
        else:
            st.error("😕 Błędny klucz produktu. Spróbuj ponownie.")
            st.session_state["password_correct"] = False
    return False

# --- BAZA KONFIGURACYJNA SQLite (przechowuje datę ślubu) ---
CONFIG_DB = "wesele_config.db"
...
def get_data_slubu():
    conn = get_config_connection()
    c = conn.cursor()
    c.execute("SELECT Wartosc FROM Ustawienia WHERE Klucz='DataSlubu'")
    result = c.fetchone()
    conn.close()
    if result:
        try:
            ...
            return date(2027, 7, 13)
    else:
        domyslna = date(2027, 7, 13)
        save_data_slubu(domyslna)
        return domyslna

def save_data_slubu(nowa_data):
    conn = get_config_connection()
    c = conn.cursor()
    data_str = nowa_data.strftime("%Y-%m-%d")
    c.execute("INSERT OR REPLACE INTO Ustawienia (Klucz, Wartosc) VALUES ('DataSlubu', ?)", (data_str,))
    conn.commit()
    conn.close()

# --- GŁÓWNA APLIKACJA (tylko po zalogowaniu) ---
if check_password():

    DATA_SLUBU = get_data_slubu()   # data z bazy

    # --- POŁĄCZENIE Z GOOGLE SHEETS (bez zmian) ---
    @st.cache_resource
    def pobierz_arkusze():
    ...
    # --- FUNKCJA GENERUJĄCA PDF (bez zmian) ---
    ...
    # --- UI APLIKACJI ---
    # Pasek boczny z ustawieniami daty
    with st.sidebar:
        st.header("⚙️ Ustawienia")
        current_date_db = get_data_slubu()
        st.write("Data ślubu:")
        new_date = st.date_input("Wybierz datę", value=current_date_db)
        if new_date != current_date_db:
            save_data_slubu(new_date)
            st.success("Zaktualizowano datę!")
            st.rerun()
        st.write("---")
        if st.button("🔴 Wyłącz aplikację"):
            st.warning("Aplikacja zostaje zamknięta. Możesz teraz zamknąć kartę przeglądarki.")
            import os
            import signal
    ...
    if dzisiaj <= DATA_SLUBU:
        ...
        st.info(f"📅 Data: **{DATA_SLUBU.strftime('%d.%m.%Y')}** | 💍 Pozostało: **{pozostalo} dni!**")
    else:
        st.success(f"📅 Data: {DATA_SLUBU.strftime('%d.%m.%Y')} | 🎉 Wesele już było! Czas na miesiąc miodowy!")
        ...
        if potwierdzeni.empty:
            ...
            if st.button("💾 Zapisz diety", key="save_diety"):
                for index, row in edited_diety.iterrows():
                    mask = df_goscie["Imie_Nazwisko"] == row["Imie_Nazwisko"]
                    df_goscie.loc[mask, "Dieta"] = row["Dieta"]
                df_save = df_goscie.copy()
                df_save["RSVP"] = df_save["RSVP"].apply(lambda x: "Tak" if x else "Nie")
                df_save["Zaproszenie_Wyslane"] = df_save["Zaproszenie_Wyslane"].apply(lambda x: "Tak" if x else "Nie")
                aktualizuj_caly_arkusz(worksheet_goscie, df_save)
                st.success("Diety zapisane!")
                st.rerun()

            st.write("---")
            st.subheader("📊 Podsumowanie diet")
