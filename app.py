...
# --- FUNKCJE POMOCNICZE (WERSJA NAPRAWIONA) ---
...
def pobierz_dane(_worksheet):
    ...
    # Używamy get_all_values zamiast get_all_records - to jest bezpieczniejsze
    ...
    # KLUCZOWA POPRAWKA: Usuwamy spacje z nazw kolumn (np. "Koszt " -> "Koszt")
...
with tab2:
    ...
    # ZABEZPIECZENIE: Sprawdzamy czy są kolumny
    ...
    # Jeśli brakuje kolumn (np. pusty arkusz), tworzymy je ręcznie
    if brakujace:
    ...
    # Konwersja danych tylko jeśli tabela nie jest pusta
    if not df_org.empty:
...
with tab3:
    ...
    if not df_td.empty:
        ...
        df_td["Termin"] = pd.to_datetime([]).dt.date
        df_td["Czy_Zrobione"] = pd.Series([], dtype=bool)
