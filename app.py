...
# ZMIANA: Zwiększono ttl z 5 na 600 sekund (10 minut).
# To zapobiegnie błędowi "Quota exceeded".
# Ponieważ używamy st.cache_data.clear() przy zapisie, dane i tak odświeżą się natychmiast po Twojej edycji.
...
def zapisz_nowy_wiersz(worksheet, lista_wartosci):
    ...
    st.cache_data.clear() # To wymusi odświeżenie danych natychmiast
...
with tab1:
    ...
    if "Zaproszenie_Wyslane" not in df_goscie.columns: df_goscie["Zaproszenie_Wyslane"] = "Nie"
...
with tab3:
    ...
    for c in ["Zadanie", "Termin", "Czy_Zrobione"]:
        if c not in df_todo.columns: df_todo[c] = ""
    ...
    st.write("---")
    df_td = df_todo.copy()
    if "Zadanie" in df_td.columns: df_td["Zadanie"] = df_td["Zadanie"].astype(str).replace("nan", "")
    if "Termin" in df_td.columns: df_td["Termin"] = pd.to_datetime(df_td["Termin"], errors='coerce').dt.date
    if "Czy_Zrobione" in df_td.columns: df_td["Czy_Zrobione"] = df_td["Czy_Zrobione"].apply(lambda x: str(x).lower() in ["tak", "true", "1"])
